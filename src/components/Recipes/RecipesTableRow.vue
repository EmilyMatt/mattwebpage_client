<template>
    ...
    <tr>
        <td style="padding-left: 12px">{{prop_index}}</td>
        <td>{{prop_recipeName}}</td>
        <td>{{prop_addedBy}}</td>
        <td>{{prop_dateAdded}}</td>
        <td>
          <img width="100" style="border-radius: 5%; box-shadow: 0 0 2px 2px black; object-fit: contain;" :src="$route.params.proxy+thumb" :alt="prop_recipeName + ' recipe thumbnail'">
        </td>
...
<script>
    ...
    mounted() {
      if (this.prop_thumb == "")
        this.thumb = "/img/noImg.jpg"
      else
        this.thumb = this.prop_thumb
    },
